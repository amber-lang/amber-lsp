...
pub fn inf_loop_parser<'a>(
    ...
    just(T!["loop"])
        ...
        .then(block_parser(stmnts, false, true))
...
pub fn iter_loop_parser<'a>(
    ...
    just(T!["for"])
        ...
        .then(
            block_parser(stmnts, false, true).recover_with(via_parser(
                default_recovery()
                    .or_not()
...
pub fn while_loop_parser<'a>(
    ...
    just(T!["while"])
        .map_with(|_, e| ("while".to_string(), e.span()))
        .then(parse_expr(stmnts.clone()).recover_with(via_parser(
            default_recovery().map_with(|_, e| (Expression::Error, e.span())),
        )))
        .then(block_parser(stmnts, false, true))
        .map_with(|((loop_keyword, exp), block), e| {
            (
                Statement::WhileLoop(loop_keyword, Box::new(exp), block),
                e.span(),
            )
        })
        .boxed()
