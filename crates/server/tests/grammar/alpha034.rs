...
use amber_grammar::alpha034::semantic_tokens::semantic_tokens_from_ast;
use amber_grammar::alpha034::{
    AmberCompiler,
    Spanned,
};
use amber_grammar::{
...
fn parse(
    ...
    Option<Vec<Spanned<amber_grammar::alpha034::GlobalStatement>>>,
    Vec<Rich<'_, String>>,
) {
    let ParserResponse {
        ast,
    ...
    let ast = match ast {
        amber_grammar::Grammar::Alpha034(ast) => ast,
...
fn parse_unwrap(
    tokens: &[Spanned<Token>],
) -> Vec<Spanned<amber_grammar::alpha034::GlobalStatement>> {
