...
use amber_types::SpannedSemanticToken;
        ...
        .flat_map(|(stmnt, span)| match stmnt {
            Statement::Block((block, _)) => match block {
                ...
                Block::Singleline(stmnt) => {
                    let stmnts = vec![*stmnt.clone()];

                    semantic_tokens_from_stmnts(&stmnts)
                }
                Block::Error => vec![],
            },
            ...
            Statement::MoveFiles(command_modifiers, (_, mv_span), src, dest, failable_handlers) => {
                let mut tokens = vec![];
                ...
                tokens.extend(semantic_tokens_from_failable_handlers(failable_handlers));
            ...
            Statement::WhileLoop((_, loop_span), expr, block) => {
                let mut tokens = vec![(
                    hash_semantic_token_type(SemanticTokenType::KEYWORD),
                    *loop_span,
                )];
...
fn semantic_tokens_from_expr((expr, span): &Spanned<Expression>) -> Vec<SpannedSemanticToken> {
    match expr {
        ...
        Expression::Command(modifiers, cmd, failable_handlers) => {
            let mut tokens = vec![];
        ...
        Expression::FunctionInvocation(modifiers, (_, name_span), args, failable_handlers) => {
            let mut tokens = vec![];
        ...
        Expression::Int(_) => vec![(hash_semantic_token_type(SemanticTokenType::NUMBER), *span)],
...
fn semantic_tokens_from_failable_handlers(
    handlers: &[Spanned<FailableHandler>],
) -> Vec<SpannedSemanticToken> {
    handlers
        .iter()
        .flat_map(|(handler, _)| {
            let mut tokens = vec![];
            match handler {
                FailableHandler::Failure((failure_handler, failure_span)) => {
                    match failure_handler {
                        FailureHandler::Handle((_, failed_span), exit_code, block) => {
                            tokens.push((
                                hash_semantic_token_type(SemanticTokenType::KEYWORD),
                                *failed_span,
                            ));

                            if let Some((_, exit_code_span)) = exit_code {
                                tokens.push((
                                    hash_semantic_token_type(SemanticTokenType::VARIABLE),
                                    *exit_code_span,
                                ));
                ...
                FailableHandler::Succeeded((_, failed_span), block) => {
                    tokens.push((
                        hash_semantic_token_type(SemanticTokenType::KEYWORD),
                        *failed_span,
                    ));
