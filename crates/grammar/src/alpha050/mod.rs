...
pub enum Block {
    ...
    Singleline(Box<Spanned<Statement>>),
    Error,
}
...
pub enum Statement {
    ...
    WhileLoop(Spanned<String>, Box<Spanned<Expression>>, Spanned<Block>),
    IterLoop(
        Spanned<String>,
...
impl LSPAnalysis for AmberCompiler {
    ...
    fn parse<'a>(&self, tokens: &'a [Spanned<Token>]) -> ParserResponse<'a> {
        ...
        ParserResponse {
            ast: Grammar::Alpha050(result.into_output()),
...
pub type RichError<'src> = Err<Rich<'src, Token, Span>>;
