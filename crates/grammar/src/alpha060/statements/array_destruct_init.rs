...
pub fn array_destruct_init_parser<'a>(
    ...
    choice((
        just(T!["let"]).map_with(|_, e| ((false, e.span()), ("let".to_string(), e.span()))),
        just(T!["const"]).map_with(|_, e| ((true, e.span()), ("const".to_string(), e.span()))),
    ))
    .then(
    ...
    .map_with(|(((is_const, keyword), names), value), e| {
        (
            Statement::ArrayDestructInit(is_const, keyword, names, value),
            e.span(),
        )
    })
    .boxed()
