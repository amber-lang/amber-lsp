...
type OpenDocumentResult = Result<(FileId, FileVersion)>;

pub trait AnalysisHost: Sync + Send {
...
#[tracing::instrument(skip_all)]
pub async fn map_import_path(uri: &Uri, path: &str, backend: &impl AnalysisHost) -> Uri {
